...
public class GlobalExceptionHandler {
    ...
    @ExceptionHandler(InvalidTripOperationException.class)
    public ResponseEntity<ErrorResponse> invalidTrip(InvalidTripOperationException ex, HttpServletRequest request) {
        return ResponseEntity.status(HttpStatus.CONFLICT).body(base(HttpStatus.CONFLICT, ex.getMessage(), request, null));
    }

    @ExceptionHandler(NoDriversAvailableException.class)
    public ResponseEntity<ErrorResponse> noDrivers(NoDriversAvailableException ex, HttpServletRequest request) {
        return ResponseEntity.status(HttpStatus.SERVICE_UNAVAILABLE)
                .body(base(HttpStatus.SERVICE_UNAVAILABLE, ex.getMessage(), request, null));
    }

    @ExceptionHandler(UpstreamUnavailableException.class)
    public ResponseEntity<ErrorResponse> upstream(UpstreamUnavailableException ex, HttpServletRequest request) {
        return ResponseEntity.status(HttpStatus.SERVICE_UNAVAILABLE)
                .body(base(HttpStatus.SERVICE_UNAVAILABLE, ex.getMessage(), request, null));
    }
